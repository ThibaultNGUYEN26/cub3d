...
/*   Updated: 2024/01/22 14:59:04 by rchbouki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* can't have anything after r,g,b besides spaces and if we've reached the end of the file => no map => error */
int	check_color_utils(char *buffer, int *k)
{
	while (buffer[*k] != '\n')
	{
		if ((buffer[*k] != '\f' && buffer[*k] != '\t' && buffer[*k] != '\r' && buffer[*k] != '\v' && buffer[*k] != ' ') || buffer[*k] == '\0')
			return (-1);
		*k += 1;		
	}
	return (1);
